...
# Create the window screen | The coordinates are the width and height of the screen
...
# Title and Icon
pygame.display.set_caption("Space Invaders.")  # Adding the title/captions
...
while running:
    ...
    for event in pygame.event.get():  # This line of code is an event function that gives the user control
        ...
        if event.type == pygame.KEYUP:  # Detects when you release keystroke
            if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:  # If left or right arrow key is pressed
    ...
    for i in range(num_of_enemies):
        ...
        if collision:  # If collision has occurred
            ...
            enemyX[i] = random.randint(0,
                                       735)  # Randomizes x coordinate of the spawn point of the enemy, after being shot
            enemyY[i] = random.randint(50,
                                       150)  # Randomizes y coordinate of the spawn point of the enemy, after being shot
